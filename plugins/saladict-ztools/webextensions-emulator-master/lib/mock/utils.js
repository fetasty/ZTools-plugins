...
export function openIframe (url, opt = {}) {
  ...
  if (!url.startsWith('http') && !url.startsWith('file://') && !urlWithoutQuery.includes('/') && urlWithoutQuery.endsWith('.html')) {
    url = 'ext-saladic/' + url;
  }
  if (!url.startsWith('http') && !url.startsWith('file://')) {
    if (window.__pluginDir) {
      url = `file://${window.__pluginDir}/${url}`;
    } else {
      url = `./${url}`;
    }
  }
