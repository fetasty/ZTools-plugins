    ...
    return new Promise((resolve, reject) => {
      ...
      listenersArea.forEach(listener => {
        ...
        if (hint === true) {
          ...
          // promise style
          isAsync = true
          hint.then(sendResponse).catch(err => {})
        }
      })
...
export function makeListener (listenersArea) {
  const add = fn => {
    if (typeof listenersArea.add === 'function') return listenersArea.add(fn)
    if (typeof listenersArea.push === 'function') return listenersArea.push(fn)
    throw new TypeError('Unsupported listeners container')
  }
  const remove = fn => {
    if (typeof listenersArea.delete === 'function') return listenersArea.delete(fn)
    if (Array.isArray(listenersArea)) {
      const idx = listenersArea.indexOf(fn)
      if (idx >= 0) listenersArea.splice(idx, 1)
      return
    }
    throw new TypeError('Unsupported listeners container')
  }
  const has = fn => {
    if (typeof listenersArea.has === 'function') return listenersArea.has(fn)
    if (Array.isArray(listenersArea)) return listenersArea.includes(fn)
    return false
  }
