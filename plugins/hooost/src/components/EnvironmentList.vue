...
<template>
  <aside class="env-list">
    ...
    >
      <span class="env-item-icon">{{ typeIcons[env.type] || '📦' }}</span>
      <span class="env-item-name">{{ env.name }}</span>
      <span v-if="env.type === 'public'" class="env-item-badge">只读</span>
      <span v-else class="env-item-count">{{ env.entries.filter(e => e.enabled).length }}</span>
    </div>
    <ContextMenu
